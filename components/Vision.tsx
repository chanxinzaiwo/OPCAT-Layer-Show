...
import { TrendingUp, Zap, Flag, MousePointerClick, Rocket } from 'lucide-react';
...
const Vision: React.FC = () => {
  ...
  const handleMilestoneClick = (targetMilestone: number) => {
    ...
    // Animation duration is 1s, so we update state at 0.8s
    setTimeout(() => {
    ...
    }, 800); // Wait for particles to travel
  };
  ...
  return (
    <section id="vision" className="py-24 bg-stone-900 relative text-white overflow-hidden border-t border-stone-800">
       <style>{`
         ...
         .animate-flow-arc {
           animation: flow-arc 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;
         }
       `}</style>
       ...
       <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          {/* LEFT: CHART AREA (Vertical Columns) */}
          <div className="bg-stone-800/50 border border-stone-700 p-8 rounded-3xl h-[550px] shadow-2xl relative group overflow-hidden flex flex-col">
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-stone-900/50 pointer-events-none"></div>
            ...
            <div className="flex justify-between items-center mb-4 relative z-10">
                ...
                {milestone === 3 && (
                    <div className="text-xs font-bold text-black bg-orange-500 px-3 py-1 rounded-full animate-bounce">
                        🎉 Liquidity Unleashed!
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 w-full relative z-10">
                ...
                {/* Liquidity Injection Animation */}
                {/* Origin: Approx position of BTC bar (Rightmost) */}
                {/* Target: Aiming generally left-downward where OP_CAT bar will be */}
                {isInjecting && (
                   <div className="absolute top-[20%] right-[10%] pointer-events-none z-30">
                       {[...Array(12)].map((_, i) => (
                           <div 
                               key={i} 
                               className="absolute w-3 h-3 bg-yellow-400 rounded-full shadow-[0_0_15px_rgba(250,204,21,1)] animate-flow-arc"
                               style={{ 
                ...
                {/* L1 Activation Effect */}
                {milestone === 3 && (
                    <div className="absolute bottom-[0] right-[10%] w-[50px] h-[90%] bg-orange-500/20 blur-xl animate-pulse pointer-events-none"></div>
                )}
            </div>
